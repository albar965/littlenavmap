...
QStringList routeStringOptionsToStr(rs::RouteStringOptions flags)
{
  ...
  ATOOLS_FLAGS_TO_STR(DIALOG_SID_STAR_NONE);
  ATOOLS_FLAGS_TO_STR(READ_ALTERNATES);
  ATOOLS_FLAGS_TO_STR(READ_MATCH_WAYPOINTS);
  ATOOLS_FLAGS_TO_STR(READ_NO_AIRPORTS);
  ATOOLS_FLAGS_TO_STR(READ_NO_TRACKS);
  ATOOLS_FLAGS_TO_STR(READ_WRITE_USERPOINT_IDENT);
  ATOOLS_FLAGS_TO_STR(REPORT);
  ATOOLS_FLAGS_TO_STR(WRITE_ALTERNATES);
  ATOOLS_FLAGS_TO_STR(WRITE_ALT_AND_SPEED);
  ATOOLS_FLAGS_TO_STR(WRITE_ALT_AND_SPEED_METRIC);
  ATOOLS_FLAGS_TO_STR(WRITE_CORTEIN_APPROACH);
  ATOOLS_FLAGS_TO_STR(WRITE_CORTEIN_DEPARTURE_RUNWAY);
  ATOOLS_FLAGS_TO_STR(WRITE_DCT);
  ATOOLS_FLAGS_TO_STR(WRITE_DEPART_AND_DEST);
  ATOOLS_FLAGS_TO_STR(WRITE_FLIGHTLEVEL);
  ATOOLS_FLAGS_TO_STR(WRITE_GFP);
  ATOOLS_FLAGS_TO_STR(WRITE_GFP_COORDS);
  ATOOLS_FLAGS_TO_STR(WRITE_NO_AIRWAYS);
  ATOOLS_FLAGS_TO_STR(WRITE_NO_FINAL_DCT);
  ATOOLS_FLAGS_TO_STR(WRITE_RUNWAYS);
  ATOOLS_FLAGS_TO_STR(WRITE_SID_STAR);
  ATOOLS_FLAGS_TO_STR(WRITE_SID_STAR_GENERIC);
  ATOOLS_FLAGS_TO_STR(WRITE_SID_STAR_SPACE);
  ATOOLS_FLAGS_TO_STR(WRITE_SKYVECTOR_COORDS);
  ATOOLS_FLAGS_TO_STR(WRITE_STAR_REV_TRANSITION);
  ATOOLS_FLAGS_TO_STR(WRITE_USR_WPT);
  ATOOLS_FLAGS_TO_STR_END;
}

ATOOLS_DEFINE_DEBUG_OPERATORS_FOR_FLAGS(RouteStringOptions, rs::RouteStringOption, routeStringOptionsToStr);
