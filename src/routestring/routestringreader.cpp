...
const static QRegularExpression
  AIRPORT_TIME_APPROACH("^(?<ap>[A-Z0-9]{3,4})" // "GCTS" Required airport
...
const static std::initializer_list<map::MapType> ROUTE_TYPES_NAVAIDS_LIST = {map::WAYPOINT, map::VOR, map::NDB};
const static std::initializer_list<map::MapType> ROUTE_TYPES_NAVAIDS_LIST_USERPT = {map::WAYPOINT, map::VOR, map::NDB, map::USERPOINT};
...
struct TypeDist
{
  TypeDist(const ageo::Pos& posParam, const ageo::Pos& lastPosParam, const ageo::Pos& destPosParam, map::MapType typeParam,
...
void RouteStringReader::initRouteTypes(rs::RouteStringOption optionsParam)
{
  options = optionsParam;
  routeTypesNavaids = map::WAYPOINT | map::VOR | map::NDB | map::USERPOINTROUTE; // USERPOINTROUTE is used for coordinate refs
  routeTypes = routeTypesNavaids | map::AIRPORT;
  routeTypesAndAirway = routeTypes | map::AIRWAY;

  if(options & rs::READ_WRITE_USERPOINT_IDENT)
  {
    routeTypesNavaids = routeTypesNavaids | map::USERPOINT;
    routeTypes = routeTypes | map::USERPOINT;
    routeTypesAndAirway = routeTypesAndAirway | map::USERPOINT;

    // Needs persistent backup since it just maintains a reference to list
    routeTypesNavaidsList = &ROUTE_TYPES_NAVAIDS_LIST_USERPT;
  }
  else
    routeTypesNavaidsList = &ROUTE_TYPES_NAVAIDS_LIST;
}

bool RouteStringReader::createRouteFromString(const QString& routeString, rs::RouteStringOption optionsParam,
                                              ...
                                              float *speedKtsParam, bool *altIncludedParam)
{
  ...
  initRouteTypes(optionsParam);

  airwayQuery->setUseTracks(!(options & rs::READ_NO_TRACKS));
  ...
  {
    // Get departure airport and probably SID ============================================
    // Insert first as departure
    QString sidExitWp; // Exit waypoint of SID returned to allow airway matching
    ...
    if(!addDestination(fp, &alternates, mapObjectRefs, cleanItems, starEntryWp))
      return false;
...
#ifdef DEBUG_INFORMATION
    qDebug() << Q_FUNC_INFO << "cleanItems 2" << cleanItems;
#endif
  ...
  if(speedKtsParam != nullptr && *speedKtsParam > 0.f)
  {
    ...
    if(atools::almostEqual(*speedKtsParam, cruise, 1.f))
      appendNote(tr("Ignoring speed instruction %1.").arg(Unit::speedKts(*speedKtsParam)));
    else
      // Detected speed differs from aircraft performance
      appendNote(tr("Ignoring speed instruction %1 in favor of aircraft performance which uses %2 true airspeed.").
  ...
  if(!(options & rs::READ_NO_TRACKS) && NavApp::hasNatTracks())
  {
  ...
  // Get a list of map results for each item ============
  ...
  for(const QString& item : std::as_const(cleanItems))
  {
    ...
    if(!result.isEmpty(routeTypesAndAirway))
    ...
    if(result.hasAirways())
    {
      ...
      else
      {
        ...
        for(const map::MapWaypoint& wp : result.waypoints)
        {
          ...
          entryBuilder->entryFromWaypoint(wp, entry, true /* resolveWaypoints */);
...
const QStringList RouteStringReader::getAllMessages() const
{
  ...
  if(!messages.isEmpty() && !noteMessages.isEmpty())
    messages << QStringLiteral(); // Converted to newline
...
map::MapRefExt RouteStringReader::mapObjectRefFromEntry(const FlightplanEntry& entry, const map::MapResult& result, const QString& name)
{
  ...
  {
    // Userpoint route (green rectangle)
    if(options & rs::READ_WRITE_USERPOINT_IDENT)
      // Test if route userpoint matches a userpoint from the database
      return result.userpoints.constFirst().getRefExt(name);
    else
      // No userpoints - store as coordinate
      return result.userpointsRoute.constFirst().getRefExt(name);
  }
  else
...
{
  map::MapType types = map::WAYPOINT | map::VOR | map::NDB | map::AIRPORT;
  if(options & rs::READ_WRITE_USERPOINT_IDENT)
    types = types | map::USERPOINT;

  MapResult newResult;
  resultWithClosest(newResult, result, nearestPos, types);
...
void RouteStringReader::resultWithClosest(map::MapResult& resultWithClosest, const map::MapResult& result,
                                          const atools::geo::Pos& nearestPos, map::MapType types)
{
  struct DistData
  {
    map::MapType type;
  ...
  {
    for(int i = 0; i < result.userpoints.size(); i++)
      sorted.append({map::USERPOINT, i, result.userpoints.at(i).getPosition()});
  }
  ...
  if(!sorted.isEmpty())
  {
    ...
    else if(nearest.type == map::USERPOINT)
      resultWithClosest.userpoints.append(result.userpoints.at(nearest.index));
  }
}
...
void RouteStringReader::insertMessage(const QString& message, int index)
{
  ...
  qInfo() << Q_FUNC_INFO << "Message:" << QTextDocumentFragment::fromHtml(message).toPlainText();
}

void RouteStringReader::appendNote(const QString& message)
{
  if(plaintextMessages)
    noteMessages.append(message);
  else
    noteMessages.append(atools::util::HtmlBuilder::noteMessage(message));
  qWarning() << Q_FUNC_INFO << "Note:" << QTextDocumentFragment::fromHtml(message).toPlainText();
}
...
void RouteStringReader::appendWarning(const QString& message)
{
  ...
  qWarning() << Q_FUNC_INFO << "Warning:" << QTextDocumentFragment::fromHtml(message).toPlainText();
}
...
void RouteStringReader::appendError(const QString& message)
{
  ...
  qWarning() << Q_FUNC_INFO << "Error:" << QTextDocumentFragment::fromHtml(message).toPlainText();
}
...
void RouteStringReader::extractAirportIdentDeparture(QString item, QString& airport, QString& runway)
{
  ...
  if(match.hasMatch())
  {
    ...
    if(!time.isEmpty())
      appendNote(tr("Ignoring time specification \"%1\" for departure airport %2.").arg(time, item));
                                                       ...
                                                       QString& runway)
{
  ...
  if(!time.isEmpty())
    appendMessage(tr("Ignoring time specification \"%1\" for destination airport %2.").arg(time, item));
}
...
void RouteStringReader::readSidAndTrans(QStringList& items, int& sidId, int& sidTransId, const map::MapAirport& departureAirport,
                                        const QString& runway)
{
  ...
  if(!foundAirway)
  {
    ...
    if(!items.constFirst().contains('.'))
    {
      ...
      sidId = procQuery->getSidId(departureAirport, sid, runway, true /* strict */);
      ...
      {
        sidTransId = procQuery->getSidTransitionId(departureAirport, trans, sidId, true /* strict */);
                                     ...
                                     QString& sidExitWp)
{
  ...
  if(departureAirport.isValid())
  {
    ...
    if(!items.isEmpty())
    {
      ...
      readSidAndTrans(items, sidId, sidTransId, departureAirport, runway);
      ...
      if(sidId != -1)
      {
        ...
        if(sidTransId == -1)
          // Only SID
          legs = procQuery->getProcedureLegs(departureAirport, sidId);
        else
          // SID and transition
          legs = procQuery->getTransitionLegs(departureAirport, sidTransId);

        if(legs != nullptr)
        {
          sidLegs = *legs;
          if(!sidLegs.isEmpty())
            sidExitWp = sidLegs.constLast().fixIdent;
        }
      }
                                       ...
                                       map::MapRefExtList *mapObjectRefs, QStringList& items, QString& starEntryWp)
{
  ...
  while(idx >= 0)
  {
    ...
    int consume = 0;
    destinationInternal(airport, star, approach, items, starEntryWp, runwayEnd, consume, idx);
    ...
    if(!star.isEmpty() || !approach.isEmpty())
    {
      ...
      destInfos.append(DestInfo(star, approach, airport, runwayEnd, !star.isEmpty() ? consume : -1));
      break;
    }

    if(airport.isValid() && (destInfos.isEmpty() || destInfos.constFirst().airport.position.distanceMeterTo(airport.position) <
                             ageo::nmToMeter(MAX_ALTERNATE_DISTANCE_NM)))
    {
      ...
      destInfos.append(DestInfo(star, approach, airport, runwayEnd, consume));
      idx--;
    }
    else
...
void RouteStringReader::readApproachAndTrans(const QString& approachArinc, const QString& approachTrans,
                                             int& approachId, int& transitionId, const map::MapAirport& destAirport)
{
                                            ...
                                            const map::MapAirport& destAirport)
{
  ...
  if(starMatch.hasMatch())
  {
    ...
    starId = procQuery->getStarId(destAirport, star, runway, true /* strict */);
    if(starId != -1 && !trans.isEmpty())
    {
      // Try tansition if STAR was found
      starTransId = procQuery->getStarTransitionId(destAirport, trans, starId, true /* strict */);
      if(starTransId == -1)
...
void RouteStringReader::readStarAndTransSpace(const QString& star, QString trans, const QString& runway, int& starId, int& starTransId,
                                              const map::MapAirport& destAirport)
{
  starId = procQuery->getStarId(destAirport, sidStarAbbrev(star), runway, true /* strict */);

  if(starId != -1 && !trans.isEmpty())
    starTransId = procQuery->getSidTransitionId(destAirport, trans, starId, true /* strict */);
}

void RouteStringReader::readStarAndTrans(QStringList& items, const QString& runway, int& starId, int& starTransId,
                                         int& consume, const map::MapAirport& destAirport, int index)
{
  starId = -1;
  starTransId = -1;
  consume = 0;

  // KVHN SSO SUNSS8 KDVT KPHX
  // destination = KPHX
  QString item1 = items.value(index - 1); // KDVT
  QString item2 = items.value(index - 2); // SUNSS8

  if(!item1.contains('.'))
  {
    // Read "TRANS STAR" notation ======================================================
    // KDVT SUNSS8 for destination = KPHX
    readStarAndTransSpace(item1 /* star */, item2 /* trans */, runway, starId, starTransId, destAirport);

    if(starId == -1)
    {
      // Avoid getting an airport from the alternates which has a STAR with the same name
      // KCNM WHOLE EWM J4 SUNSS8 SSO KDVT KPHX
      if(!airportQuerySim->getAirportByIdent(item1.section('/', 0, 0)).isValid())
      {
        // Read "STAR TRANS" notation ======================================================
        // SUNSS8 KDVT for destination = KPHX
        readStarAndTransSpace(item2 /* star */, item1 /* trans */, runway, starId, starTransId, destAirport);
      }
    }
  }
  ...
  if(starId == -1)
  {
    ...
    readStarAndTransDot(item1 % '.' % item2, runway, starId, starTransId, destAirport);

    if(starId == -1)
      // Read "TRANS.SID" notation ======================================================
      readStarAndTransDot(item2 % '.' % item1, runway, starId, starTransId, destAirport);

    if(starId != -1)
      // Do not remove items yet from list
      consume = 1;
  }
  else
  {
...
void RouteStringReader::destinationInternal(map::MapAirport& destAirport, proc::MapProcedureLegs& starLegs,
                                            proc::MapProcedureLegs& approachLegs, QStringList& items,
                                            QString& starEntryWp, map::MapRunwayEnd& runwayEnd, int& consume, int index)
{
  ...
  airportSim(destAirport, airportIdent);

  // Check if list is large enough to hold a destination
  if(destAirport.isValid() && !items.isEmpty() && index >= 0)
  {
    // ========================================================================
    ...
    if(!approach.isEmpty())
    {
      ...
      readApproachAndTrans(approach, transition, approachId, transitionId, destAirport);

      if(transitionId != -1)
      {
        // Approach and transition - fetch both
        const proc::MapProcedureLegs *legs = procQuery->getTransitionLegs(destAirport, transitionId);
        if(legs != nullptr)
          approachLegs = *legs;
      }
      ...
      {
        // Only approach
        const proc::MapProcedureLegs *legs = procQuery->getProcedureLegs(destAirport, approachId);
        if(legs != nullptr)
          approachLegs = *legs;
      }
    ...
    else if(!runway.isEmpty())
    {
      ...
      runwayEnd = airportQuerySim->getRunwayEndByName(destAirport.id, runway);
      // mapQuery->getRunwayEndByNameFuzzy(runwayEnds, runway, airport, true /* navdata */);
    ...
    readStarAndTrans(items, runway, starId, starTransId, consume, destAirport, index);

    if(starId != -1 && starTransId == -1)
    {
      // Only STAR
      const proc::MapProcedureLegs *legs = procQuery->getProcedureLegs(destAirport, starId);
      if(legs != nullptr)
      {
        starLegs = *legs;
        if(!starLegs.isEmpty())
          // Remember entry waypoint to add back later for airway matching
          starEntryWp = legs->constFirst().fixIdent;
      }
    }
    else if(starId != -1 && starTransId != -1)
    {
      // STAR and transition
      const proc::MapProcedureLegs *legs = procQuery->getTransitionLegs(destAirport, starTransId);
      if(legs != nullptr)
      {
        starLegs = *legs;
        if(!starLegs.isEmpty())
...
void RouteStringReader::findWaypoints(MapResult& result, const QString& item, bool matchWaypoints)
{
  ...
  mapQuery->getMapObjectByIdent(result, routeTypesAndAirway, item, QStringLiteral(), QStringLiteral(),
                                false /* airportFromNavdatabase */, map::AP_QUERY_ALL);
...
QStringList RouteStringReader::cleanItemList(const QStringList& items, float& speedKnots, float& altFeet)
{
  ...
  for(int i = 0; i < items.size(); i++)
  {
    ...
    if(item == "SID")
    {
      appendNote(tr("Replacing plain SID instruction with DCT (direct)."));
      continue;
    }

    if(item == "STAR")
    {
      appendNote(tr("Replacing STAR instruction with DCT (direct)."));
      continue;
    }
    ...
    if(match.hasMatch())
    {
      ...
      appendNote(tr("Ignoring speed and altitude at waypoint %1. "
...
{
  resultList.erase(std::remove_if(resultList.begin(), resultList.end(), [this](const ParseEntry& type) -> bool {
    return type.result.isEmpty(routeTypesAndAirway);
  }), resultList.end());
}
