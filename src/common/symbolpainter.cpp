...
QIcon SymbolPainter::createAirportMsaIcon(const map::MapAirportMsa& airportMsa, const QFont& font, float symbolScale, int *actualSize)
{
  ...
  size = airportMsaSize(&painterTemp, airportMsa, symbolScale);
  ...
  SymbolPainter().drawAirportMsa(&painter, airportMsa, size / 2, size / 2, symbolScale, false);
  return QIcon(pixmap);
}
...
int SymbolPainter::airportMsaSize(QPainter *painter, const map::MapAirportMsa& airportMsa, float sizeFactor)
{
  return painter->fontMetrics().horizontalAdvance("0000") *
         atools::roundToInt(airportMsa.altitudes.size() > 1 ? sizeFactor : sizeFactor / 2.f);
}

void SymbolPainter::drawAirportMsa(QPainter *painter, const map::MapAirportMsa& airportMsa, float x, float y, float symbolScale,
                                   bool header)
{
  atools::util::PainterContextSaver saver(painter);

  painter->setPen(QPen(mapcolors::msaSymbolColor, 2, Qt::SolidLine, Qt::FlatCap));
  painter->setBackground(mapcolors::msaFillColor);
  painter->setBrush(mapcolors::msaFillColor);
  painter->setBackgroundMode(Qt::OpaqueMode);

  int size = airportMsaSize(painter, airportMsa, symbolScale);
  ...
  // Opaque fill
  QColor fill = mapcolors::msaFillColor;
  fill.setAlpha(255);
