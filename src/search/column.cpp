...
Column& Column::filter(bool value, QAction *showOptionsActionParam)
{
  colCanBeFiltered = value;
  showOptionsAction = showOptionsActionParam;
  return *this;
}
...
bool Column::isWidgetEnabled() const
{
  if(colWidget != nullptr)
    return colWidget->isEnabled();
  else if(colMinWidget != nullptr && colMaxWidget != nullptr)
    return colMinWidget->isEnabled() && colMaxWidget->isEnabled();
  else
    return false;
}
