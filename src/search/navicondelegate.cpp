...
void NavIconDelegate::paint(QPainter *painter, const QStyleOptionViewItem& option, const QModelIndex& index) const
{
  ...
  if(type == map::WAYPOINT)
    ...
    symbolPainter->drawWaypointSymbol(painter, QColor(), x, y, symbolSize * 0.7f, 1.f, sf::FILL_NONE);
  else if(type == map::NDB)
    symbolPainter->drawNdbSymbol(painter, x, y, symbolSize, 1.f, sf::FILL_NONE, false, NavApp::isGuiStyleDark());
  else if(type == map::VOR)
  {
    ...
    symbolPainter->drawVorSymbol(painter, vor, x, y, symbolSize * 0.9f, 1.f, 0.f, sf::FILL_NONE, false /* fast */,
                                 NavApp::isGuiStyleDark());
  }
}
