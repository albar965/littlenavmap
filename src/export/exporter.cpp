...
void Exporter::createVisualColumnIndex(int cnt, QList<int>& visualToIndex)
{
  visualToIndex.clear();
...
QStringList Exporter::headerNames(int cnt, const QList<int>& visualToIndex)
{
...
void Exporter::fillRecord(const QVariantList& values, const QStringList& cols, QSqlRecord& rec)
{
  ...
  if(rec.isEmpty())
    for(int i = 0; i < values.size(); i++)
      rec.append(QSqlField(cols.at(i), values.at(i).metaType()));

  for(int i = 0; i < values.size(); i++)
  {
    QVariant val = values.at(i);
    if(val.metaType() == QMetaType::fromType<QString>() && val.toString().isEmpty())
      rec.setNull(i);
    else
